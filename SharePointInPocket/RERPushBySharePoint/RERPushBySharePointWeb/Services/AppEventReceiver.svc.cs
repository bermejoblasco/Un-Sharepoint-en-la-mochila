    ...
    public class AppEventReceiver : IRemoteEventService
    {
            ...
            try
            {
                using (ClientContext clientContext = TokenHelper.CreateAppEventClientContext(properties, false))
                {
                    if (clientContext != null)
                    {
                        ...
                        {
                            if (receiver.ReceiverName == "RERPushNotificationWeb")
                            {
                                isReRExsist = true;
                                break;
                            }
                        ...
                        if (!isReRExsist)
                        {
                            ...
                            https://localhost:44304/Services/RERSpsMadPushNotification.svc
                            EventReceiverDefinitionCreationInformation eventReDefCreation = new EventReceiverDefinitionCreationInformation()
                            {
                                ...
                                ReceiverName = "RERPushNotificationWeb",
                                ReceiverClass = "RERPushNotificationWeb",
        ...
        private void HandleAppUnistalled(SPRemoteEventProperties properties)
        {
            using (ClientContext clientContext = TokenHelper.CreateAppEventClientContext(properties, false))
            {
                ...
                {
                    if (erdef.ReceiverName == "RERPushNotificationWeb")
                    {
                        toDelete.Add(erdef);
                    }
                }
